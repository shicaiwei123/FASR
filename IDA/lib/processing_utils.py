...
def face_recrop(face_dir, face_recrop_dir, save_mode='.bmp'):
    '''
    face re-crop with landmarks
    :param face_dir:人脸检测之后的人脸所在文件夹
    :param face_recrop_dir:
    :return:
    '''
    ...
    for root, dirs, files in os.walk(face_dir):
        ...
        if dirs == []:
            ...
            for file_name in files:
                ...
                # 获取存储路径
                frame_dir_split = face_recrop_dir.split('/')
                file_path_split = file_path.split('/')
                file_path_split.pop()  # 去掉文件名
                sub_split = [item for item in file_path_split if item not in frame_dir_split]
                sub_split = sub_split[1:3]
                save_dir = face_recrop_dir
                for item in sub_split:
                    save_dir = os.path.join(save_dir, item)
                if not os.path.exists(save_dir):
                    os.makedirs(save_dir)
